...
def check(name):
    data = requests.get("https://docs.google.com/spreadsheets/d/<google sheet id>/gviz/tq?tqx=out:csv&tq=SELECT *")
...
post_db = f"https://script.google.com/macros/s/<app script web app>/exec?url={url_textbox}&Unique_id={Id_textbox}"
if st.button("Shorten"):
    if check(Id_textbox) == "True":
        st.error("choose another Id")
    else:
        requests.get(post_db)
        shortned_url = f"https://<heroku api app name>.herokuapp.com/{Id_textbox}"
